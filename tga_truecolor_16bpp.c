...
/*   Updated: 2019/04/24 11:46:38 by fmessina         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
												...
												const int src)
{
	...
	if (tga)
	{
		...
		tga->pixels[dst] = (argb[0] << 24) + (argb[1] << 16) + (argb[2] << 8) | argb[3];
		return (true);
	}
		return (tga_berror("NULL TGA pointer in TC16S!", tga));
}
...
bool				tga_truecolor_16bpp(t_tga *tga)
{
	...
	if (tga)
	{
		...
		return (tga_berror("NULL TGA pointer in TC16!", tga));
}
